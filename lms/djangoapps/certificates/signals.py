...
def fire_ungenerated_certificate_task(user, course_key, expected_verification_status=None):
    ...
    if is_using_certificate_allowlist(course_key):
        log.info('{course} is using allowlist certificates. Attempt will be made to generate an allowlist certificate '
                 'for {user}.'.format(course=course_key, user=user.id))
